...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        if epsilon < random.uniform(0,1):
            ...
            rand0m = random.randint(0, len(all_actions)-1)
            action = all_actions[rand0m]
            #print("acc" , action)
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.1
        ...
        # It is recommended you use the np.random module
        #action = None
        epsilonz = epsilon_final-epsilon_initial
        epsdelt = epsilon_initial + epsilonz * (current_total_steps / anneal_timesteps)
        #print("HIIIIIIIIT?" ,epsdelt)
        if epsdelt < random.uniform(0,1):
            action = np.nanargmax(Q[state])
        else:
            rand0m = random.randint(0, len(all_actions)-1)
            action = all_actions[rand0m]
       ...
       # bestval = -1000
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                # selection of action
                list_pos = self.allowed_moves[s_current] #possible actions

                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
                # Chose an action from all possible actions
                #action = np.nanargmax(Q[s_current]) # choose possible action at current position #Take out highest index in the array
                #def epsilon_greedy(Q,
                   #state,
                   #all_actions,
                   #current_total_steps=0,
                ...
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
                Q[s_current][action] = Q[s_current][action] + lr*(R + np.nanmax(Q[s_next])- Q[s_current][action]) 
